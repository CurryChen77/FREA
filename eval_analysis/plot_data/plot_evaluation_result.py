...
from frea.util.logger import colorprint
...
def draw_data(All_data, data_name, save_dir, bins, baseline_CBV='Standard', density=True, show=False):
    ...
    for ego_type, datas in All_data_per_ego.items():
        ...
        save_name = osp.join(save_dir, f'{ego_type}-{data_save_name}.png')
...
def plot_metric(result, name, activate_per=True):
    colorprint('>> ' + '-' * 20 + str(name) + '-' * 20, color='green')
...
def process_record(args):
    ...
    show = args.show
    eval_dirs = list_directories(osp.join(ROOT_DIR, 'eval_analysis/processed_data'))
    for eval_dir in eval_dirs:
        # plot all the result of in the evaluation dir
        colorprint('>> ' + '-' * 14 + f'Processing {eval_dir}' + '-' * 14, color='red')
        base_dir = os.path.join(osp.join(ROOT_DIR, 'eval_analysis/processed_data', eval_dir))
        if eval_dir.startswith('eval_ego'):
            print('>> not processing ego evaluation')
        else:
            save_dir = os.path.join(ROOT_DIR, 'eval_analysis', 'figures', eval_dir)
            os.makedirs(save_dir, exist_ok=True)
            if len(os.listdir(save_dir)) > 0 and not overload:
                ...
                if 'collision_traj' in args.data:
                    ...
                    draw_data(collision_impulse, 'Collision Impulse (kN·s)', save_dir, bins=collision_impulse_bins, baseline_CBV='PPO', show=show)
                    # collision velocity info
                    collision_vel_bins = np.linspace(0, 10, 20)
                    draw_data(collision_vel, 'Collision Velocity (m/s)', save_dir, bins=collision_vel_bins, baseline_CBV='PPO', show=show)

                if 'all_traj' in args.data:
                    # PET distribution
                    PET_bins = np.linspace(0, 3, 30)
                    draw_data(PET_all_data, 'Post Encroachment Time (s)', save_dir, bins=PET_bins, baseline_CBV='Standard', show=show)

                    # TTC distribution
                    TTC_bins = np.linspace(0, 5, 20)
                    draw_data(TTC_all_data, 'Time to Collision (s)', save_dir, bins=TTC_bins, baseline_CBV='Standard', show=show)

                    # ego distance distribution
                    ego_dis_bins = np.linspace(0, 10, 30)
                    draw_data(ego_dis_data, 'Distance to Ego Vehicle (m)', save_dir, bins=ego_dis_bins, baseline_CBV='Standard', show=show)

                    plot_metric(infeasible_ratio, 'Infeasible Ratio (%)')
                    fea_bins = np.linspace(0, 10, 20)
                    draw_data(feasibility_Vs, 'Feasibility Values', save_dir, bins=fea_bins, baseline_CBV='PPO', show=show)
...
def process_result(args):
    ...
    for eval_dir in eval_dirs:
        ...
        colorprint('>> ' + '-' * 14 + f'Finish {eval_dir}' + '-' * 14, color='red')
