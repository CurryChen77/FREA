...
def plot_feasibility_data_distribution(args):
    ...
    axs[0].set_xlabel('Relative X Coordinate of BVs (m)', fontsize=13)
    axs[0].set_ylabel('Relative Y Coordinate of BVs (m)', fontsize=13)
    cb1 = fig.colorbar(x_y_img, ax=axs[0], pad=0.01)
    cb1.set_ticks([])
    cb1.ax.tick_params(axis='both', which='both', length=0)
    cb1.ax.text(1.5, 0, 'Low', ha='left', va='bottom', transform=cb1.ax.transAxes, fontsize=12)
    cb1.ax.text(1.5, 1, 'High', ha='left', va='top', transform=cb1.ax.transAxes, fontsize=12)
    ...
    axs[1].set_xlabel('Minimum Distance between AV and BVs (m)', fontsize=13)
    ...
    axs[2].set_xlabel('Relative Yaw of BVs (rad)', fontsize=13)
    axs[2].set_ylabel('Absolute Speed of BVs (m/s)', fontsize=13)
    cb2 = fig.colorbar(yaw_speed_img, ax=axs[2], pad=0.01)
    cb2.set_ticks([])
    cb2.ax.tick_params(axis='both', which='both', length=0)
    cb2.ax.text(1.5, 0, 'Low', ha='left', va='bottom', transform=cb2.ax.transAxes, fontsize=12)
    cb2.ax.text(1.5, 1, 'High', ha='left', va='top', transform=cb2.ax.transAxes, fontsize=12)

    _, _, _, throttle_steering_angle_img = axs[3].hist2d(throttle, steering_angle, bins=45, cmap=cmap, norm=LogNorm(), alpha=0.9)
    axs[3].set_xlabel('AV Throttle', fontsize=13)
    axs[3].set_ylabel('AV Steering Angle (rad)', fontsize=13)
    ...
    cb3.ax.text(1.5, 0, 'Low', ha='left', va='bottom', transform=cb3.ax.transAxes, fontsize=12)
    cb3.ax.text(1.5, 1, 'High', ha='left', va='top', transform=cb3.ax.transAxes, fontsize=12)

    axs[0].text(0.5, -0.2, r'(a)', ha='center', va='top', transform=axs[0].transAxes, fontsize=18)
    axs[1].text(0.5, -0.2, r'(b)', ha='center', va='top', transform=axs[1].transAxes, fontsize=18)
    axs[2].text(0.5, -0.2, r'(c)', ha='center', va='top', transform=axs[2].transAxes, fontsize=18)
    axs[3].text(0.5, -0.2, r'(d)', ha='center', va='top', transform=axs[3].transAxes, fontsize=18)

    fig.subplots_adjust(wspace=0.01, bottom=0.15)
    plt.tight_layout()
