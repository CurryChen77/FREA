...
class CarlaBehaviorAgent(BasePolicy):
    ...
    def get_action(self, obs, infos, deterministic=False):
        ...
        for e_i in infos:
            ...
            # TODO the waypoint list in frea and carla's behavior agent is different
            # for the behavior agent, the goal may be reached (no more waypoints to chase), but frea still got waypoints
            if controller.done():
                throttle = 0.1
                steer = 0
                brake = 0
